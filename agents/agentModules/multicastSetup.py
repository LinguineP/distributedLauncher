...
class MulticastSocket:
        ...
        if sys.platform == "win32":
            for iface_ip in self.iface_ips:
                new_socket = socket.socket(
                    family=self.addr_family, type=socket.SOCK_DGRAM
                )
    ...
    def recvfrom(
        self, bufsize: int = 4096, flags: int = 0
        ...
        return cast(
            Tuple[bytes, Tuple[str, int]], read_list[0].recvfrom(bufsize, flags)
        )
        ...
        for multicast_addr in multicast_ip:
            if sys.platform == "win32":
                ...
                multicast_socket.setsockopt(
                    socket.IPPROTO_IP, socket.IP_ADD_MEMBERSHIP, packed_mreq
                )
