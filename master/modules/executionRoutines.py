...
def startScripts(params_dict):
    ...
    masterNode=[]
    if msg.masterIp:
        masterNode=list(filter(lambda node: node['ip']==msg.masterIp ,selectedNodes))
        selectedNodes=list(filter(lambda node: node['ip']!=msg.masterIp ,selectedNodes))
        msg.send_start(dest_ip=masterNode[0]['ip'],script=selectedScript,numberOfNodes=selectedNumberOfNodes,nodeId=0,masterNodeId=0,masterNodeIp=masterNode[0]['ip'],decent=False)
        nodeNo=1
    ...
    for node in selectedNodes:
        ...
        msg.send_start(dest_ip=node['ip'],script=selectedScript,numberOfNodes=selectedNumberOfNodes,nodeId=0,masterNodeId=0,masterNodeIp=masterNode[0]['ip'],decent=False)
        nodeNo+=1
